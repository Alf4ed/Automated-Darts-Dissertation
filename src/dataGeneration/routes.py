...
def find_nearest_region(x, y):
    queue = [[x,y]]

    while True:
        pos = queue.pop(0)
        aim = display.score_dart(pos[0], pos[1])
...
for i in range(2, 180):
    ...
    if i not in normal.IMPOSSIBLE_CHECKOUTS[2]:
        ...
        aim = find_nearest_region(max_index[1]-200, 200-max_index[0])
        out1 = aim.to_string()
        ...
        if i-aim.value() == 0:
            ...
            aim2 = find_nearest_region(max_index2[1]-200, 200-max_index2[0])
            out2 = aim2.to_string()
            ...
            if i-aim.value()-aim2.value() == 0:
                ...
                aim3 = find_nearest_region(max_index3[1]-200, 200-max_index3[0])
                out3 = aim3.to_string()
            else:
                out3 = "IMPOSSIBLE"
        else:
