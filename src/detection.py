...
def findDartTip(before, after, pos):
    ...
    ret, thresh = cv2.threshold(norm_img1, 150, 255, cv2.THRESH_BINARY)
...
rootdir = 'C:/Users/alfre/Documents/cs310/images/labeled_images/'

for subdir, dirs, files in os.walk(rootdir):
    for file in files:
        if re.search("x", file) is not None:
            board = Dartboard(subdir)
            board.drawPoint(0, -354)
            board.drawPoint(-354, 0)

            xBefore = cv2.imread('C:/Users/alfre/Documents/cs310/images/helper_images/empty/x_0.jpg')
            xAfter = cv2.imread(os.path.join(subdir, file))
            xX, xY = findDartTip(xBefore, xAfter, 2)

            xGrad = rotateGradient(angleToGradient(findAngle(xX, xAfter, xFov, 1558)))

            fileY = re.sub('x', 'y', file)
            yBefore = cv2.imread('C:/Users/alfre/Documents/cs310/images/helper_images/empty/y_0.jpg')
            yAfter = cv2.imread(os.path.join(subdir, fileY))
            yX, yY = findDartTip(yBefore, yAfter, 3)

            yGrad = angleToGradient(findAngle(yX, yAfter, yFov, 1448))

            board.drawLine(0, -354, xGrad)
            board.drawLine(-354, 0, yGrad)

            x, y = board.intersect(0, -354, xGrad, -354, 0, yGrad)
            r, theta = board.cartesianToPolar(x, y)
            board.score(r, theta)
            print(subdir)
